...
class TransformerModelOutputExcess(nn.Module):
    def __init__(
            self,
            hops,
            n_class,
        ...
        self.att_embeddings_nope = nn.Linear(self.hidden_dim, self.hidden_dim)
        self.input = nn.EmbeddingBag(self.input_dim, self.hidden_dim, mode='sum', include_last_offset=True)
        ...
        self.out_proj = nn.Sequential(
            nn.Linear(self.hidden_dim, int(self.n_class / 4)),
            nn.ReLU(),
            nn.Linear(int(self.n_class / 4), int(self.n_class / 2)),
            nn.ReLU(),
            nn.Linear(int(self.n_class / 2), self.n_class),
        )
    ...
    def forward(self, batch_x):
        tensor = self.att_embeddings_nope(batch_x)
